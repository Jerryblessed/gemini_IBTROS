...
class Worker(threading.Thread):
    ...
    def __order_menu(self):
        ...
        self.bot.send_message(self.chat.id, self.loc.get("ask_destination"), reply_markup=cancel)
        # Wait for user input
        ...
        a = f"""Answer
            ...
            Avoid providing responses in formats like "<Product EIE>" or "<Product I want to go to Mountain of fire (AKA MFM)>". Instead, return only the matched entry text without brackets.

            Example:
            - Query: 'I want to go to Mountain of fire (AKA MFM)'
            - Output: 'Destination-route:Dawko MFM(Dwako, Abuja, Nigeria) JRJ Excellet academy(Aldenco Abuja, Nigeria)
                        Time-frame: 8:00AM-6:00PM
                        User Name: Mr. David Ope
                        username: @david bright' (from the product list)

            - Query: 'EEIE'
            - Output: 'EIE' (from the product list)
